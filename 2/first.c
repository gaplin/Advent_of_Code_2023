...
char is_valid(game *game) {
    for(int i = 0; i < game->num_of_sequences; ++i) {
        for(int j = 0; j < game->sequences[i].num_of_balls_showings; ++j) {
            balls_shown balls = game->sequences[i].balls[j];
            if(balls.color == RED && balls.count > 12) {
                return 0;
            }
            if(balls.color == BLUE && balls.count > 14) {
                return 0;
            }
            if(balls.color == GREEN && balls.count > 13) {
                return 0;
            }
        }
    }
